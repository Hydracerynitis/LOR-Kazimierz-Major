...
namespace KazimierzMajor
{
    ...
    static class LateAttackHP
    {
        ...
        [HarmonyPrefix]
        [HarmonyPriority(Priority.LowerThanNormal)] //just to lower chance of conflict with badly written patches
        static bool StageController_StartParrying_Pre(StageController __instance, BattlePlayingCardDataInUnitModel cardA, BattlePlayingCardDataInUnitModel cardB, bool __runOriginal)
        {
            if (!__runOriginal) //something already either skipped parrying or "overwritten" - do nothing to not cause unintended effects!
            {
                return false;
            }
            if (!(IsLateAttack(cardA) || IsLateAttack(cardB)) || cardB.isKeepedCard) //if both are normal cards, do nothing
            {
                return true;
            }
